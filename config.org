...
#+begin_src emacs-lisp :results file :file config_compile.log :output-dir log/
  (let ((current_dir (file-name-directory (buffer-file-name))))
    ...
    (native-compile (concat current_dir "init.el"))
    (native-compile (concat current_dir "early-init.el"))
    (native-compile (concat current_dir "lisp/custom-azik.el"))
    (native-compile (concat current_dir "plugins/ef-themes/ef-frost-theme.el"))
    (cd current_dir))
#+end_src
