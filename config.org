...
#+begin_src emacs-lisp :tangle early-init.el
  (defconst my/IS_MAC (eval-when-compile (eq system-type 'darwin)))
  (defconst my/IS_LINUX (eval-when-compile (memq system-type '(gnu gnu/linux gnu/kfreebsd berkeley-unix))))
  (defconst my/IS_WINDOWS (eval-when-compile (memq system-type '(cygwin windows-nt ms-dos))))
